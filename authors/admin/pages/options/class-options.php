...
/**
 * rcwdNggauthorsAdminOptions
 *
 *  
 */
class rcwdNggauthorsAdminOptions{
    ...
    function __construct(){
		global $rcwdNggauthors;
		add_action('nggdate_after_title', array($this, 'message_after_title'));
		if (!empty($_POST)) $this->update();
		echo $rcwdNggauthors->rcwdNggauthorsAdmin->get_output(dirname(__FILE__).'/template.php');
    }
		...
		if(isset($_GET['action'])){
			switch($_GET['action']){
				case 'updated':
					...
					$rcwdNggauthors->rcwdNggauthorsAdmin->rshow_message($message);
					break;	
			}
		}
	}	 
